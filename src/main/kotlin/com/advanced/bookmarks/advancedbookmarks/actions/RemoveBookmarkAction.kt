...
class RemoveBookmarkAction : AnAction("Remove Bookmark") {
    private class ConfirmRemoveDialog(parent: Component, private val bookmarkName: String) : DialogWrapper(parent, true) {
        init {
        ...
        override fun createCenterPanel(): JComponent {
            return JPanel().apply {
                add(JLabel("Are you sure you want to remove the bookmark '$bookmarkName'?"))
            }
        }
    }
    ...
    override fun actionPerformed(event: AnActionEvent) {
        ...
        val searchField = JBTextField().apply {
            ...
            emptyText.text = "Search bookmarks to remove..."
        }
        ...
        // Enter key navigation (remove with confirmation)
        fun tryRemoveSelectedBookmark() {
            val selectedBookmark = bookmarkList.selectedValue
            if (selectedBookmark != null) {
                popup?.closeOk(null)
                val dialog = ConfirmRemoveDialog(mainPanel, selectedBookmark.name)
                if (dialog.showAndGet()) {
                    manager.removeBookmark(selectedBookmark.name)
                    (bookmarkList.model as DefaultListModel<StringBookmarkManager.Bookmark>).removeElement(selectedBookmark)
                    updatePreview(null)
                }
            }
        }

        bookmarkList.inputMap.put(javax.swing.KeyStroke.getKeyStroke("ENTER"), "remove")
        bookmarkList.actionMap.put("remove", object : javax.swing.AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent?) {
                tryRemoveSelectedBookmark()
            }
        })

        // Double-click navigation (remove with confirmation)
